...
with col_format:
    output_format = st.selectbox("Output format", ["plain text","email","CRM note","slide outline"], index=0)
...
with st.sidebar:
    ...
    signal_snippets = st.text_area("Paste comms snippets (optional)")
...
if st.button("Generate prompt"):
    ...
    st.download_button("Download prompt (.txt)", shaped.replace("{today}", str(date.today())), file_name=fname, mime="text/plain")

st.caption("This app is rule-based, multilingual, and uses no external APIs. Safe for internal drafting.")
